        ...
        def receive():
            ...
            while True:  # Receive function
                ...
                if message=='no_text':
                    ...
                    while check_v_file:
                        if os.path.exists(f'received_file_{i}.{file_pas[len(file_pas)-1]}'):
                            i+=1
                        else:
                            check_v_file=False

                    save_as = f"received_file_{i}.{file_pas[len(file_pas)-1]}"
                    save_as_list.append(save_as)
                    def file_open(num):
                        filename= save_as_list[num]
                ...
                # Save the received file to disk
                    ...
                    button = Button(
                        ...
                        command= lambda m=c-1: file_open(m)
                    )
                    txt.window_create(END, window=button)
                    txt.insert(END,'\n')
                    step_message+=2
                    if client_check==nickname:
                        button.config(bg='#5EE87D' , text='sent file')
                    ...
                    def handel(num):
                        ...
                        if paused:
                            ...
                            threading.Thread(target=lambda : play_voice(num)).start()
                    ...
                    button =Button(
                        ...
                        highlightthickness=0,
                        font='timesnewroman 14',
                        command= lambda m=c-1: handel(m)
                    )
                    txt.window_create(END, window=button)
                    txt.insert(END,'\n')
                    step_message+=2
                    if client_check==nickname:
                        button.config(bg='#5EE87D' , text=f'sent voice')
                elif message!='':
                    client_check=client.recv(1024).decode('latin-1')
                    txt.insert(END,'\n')
                    lbl =Label(
...
def sing_up(): # function for sing up 
    ...
    def main_sing(): # function for open new window
        if len(pass_input.get())<8:
            ...
            def write():
                global check_voice     # Send function
                while True:
                    client_message=e.get()

                    if "file" not  in client_message and 'voice' not in client_message:
                        message= f'{nickname}: {client_message}'
                        client.send(message.encode('ascii'))
                        with open('client_messages.txt' , 'a') as writer:
                            writer.write(f'{message}\n')
                    elif client_message=='file':
                        filename = easygui.fileopenbox()
                        client.send('file'.encode('ascii'))
                    # Open the file in binary mode
